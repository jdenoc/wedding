...
<?php } elseif(isset($_GET['gift'])) {
    $gifts = $db->getRow("SELECT * FROM gifts WHERE id=:id", array('id'=>$ID));
    ?>
    <form action="res/update.php?gift=njsdkbnk&o=fbahjkvgbkasdvbskdv" method="post" name="edit_gift">
        <table border="0" id="edit_guest">
            <tr>
                <th colspan="4">Edit Gift <?php echo $ID; ?></th>
            </tr><tr>
                <td colspan="4"><div class="sexy_line"></div></td>
            </tr><tr>
                <td colspan="2"><label for="giver">Gift Giver:</label></td>
                <td colspan="2">
                    <input type="text" name="giver" id="giver" value="<?php echo $gifts['gift_giver']; ?>" maxlength="100" />
                </td>
            </tr><tr>
                <td class="td_top" colspan="2"><label for="gift">Gift:</label></td>
                <td colspan="2">
                    <textarea name="gift" id="gift" maxlength="255"><?php echo $gifts['gift']; ?></textarea>
                </td>
            </tr><tr>
                <td colspan="2">Thanked:&nbsp;&nbsp;</td>
                <td colspan="2">
                    <label>Yes<input type="radio" name="thanked" <?php if($gifts['thanked']==1) echo 'checked'; ?> value="1" /></label>
                    <label>No<input type="radio" name="thanked" <?php if($gifts['thanked']==0) echo 'checked'; ?> value="0" /></label>
                </td>
            </tr><tr>
                <td colspan="2"><label for="thanked_stamp">Sent Thank-You:</label></td>
                <td colspan="2">
                    <input type="date" name="thanked_stamp" id="thanked_stamp" value="<?php echo $gifts['thanked_stamp']; ?>" maxlength="10" placeholder="YYYY-MM-DD" />
                </td>
            </tr>
            <tr>
                <td colspan="4"><div class="sexy_line"></div></td>
    ...
    <form action="res/update.php?o=fbahjkvgbkasdvbskdv" method="post" name="edit_guest">
        <table border="0" id="edit_guest">
            <tr>
                ...
                <td colspan="2">
                    <label>N/A<input type="radio" name="coming" <?php if($details['coming']==-1) echo 'checked'; ?> value="-1" /></label>
                    <label>Yes<input type="radio" name="coming" <?php if($details['coming']==1) echo 'checked'; ?> value="1" /></label>
